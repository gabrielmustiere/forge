---
name: autopilot
description: Pilote autonome bout-en-bout de `/forge:feature-implem`, `/forge:refactor-implem` et `/forge:tech-implem` — délègue chaque sous-tâche à un subagent isolé, trace dans `.autopilot.json` (reprise possible), s'arrête uniquement aux stop-points stratégiques.
user_invocable: true
...
Cette skill est un **point d'entrée slash** pour le subagent `forge:autopilot`. Toute la logique d'autopilotage (phases, stop-points, reprise via `.autopilot.json`) vit dans l'agent. Le rôle de la skill se limite à : capter les arguments utilisateur, déléguer au subagent, relayer le résumé final.

L'intérêt de ce wrapper est double :
- te donner un point d'entrée slash explicite (`/forge:autopilot`) plutôt qu'un déclenchement implicite par description
...
4. Quand le subagent rend la main, relaie à l'utilisateur :
   ...
   - les prochaines étapes attendues (typiquement `/forge:review`, `/forge:commit`, `/forge:report-and-sync`)

## Référence

La spécification complète (phases, stop-points, reprise via `.autopilot.json`) vit dans la définition du subagent `forge:autopilot`, que tu invoques par son nom — tu n'as aucun fichier à lire manuellement.
