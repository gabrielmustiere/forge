...
> ⚠️ **Architecture inline obligatoire.** Tu n'invoques **jamais** le tool `Skill` pour exécuter `forge:report` ou `forge:sync`. L'indirection a déjà cassé l'écriture du `report.md` par le passé (skill non visible dans le contexte subagent, substitutions `${CLAUDE_SKILL_DIR}` non résolues, état d'avancement perdu). Toute la procédure vit dans cet agent, avec tes propres outils (`Read`, `Write`, `Edit`, `Bash`, `AskUserQuestion`).

Les SKILL.md `forge:report` et `forge:sync` restent la **référence canonique** des deux procédures (pour l'usage `/forge:report` direct). Cet agent réimplémente leur enchaînement sans Skill tool pour fonctionner de manière fiable en contexte délégué.
...
Vérifie que `${STORY_DIR}/plan.md` existe (`Read` court). Pour `STORY_TYPE = f`, vérifie aussi `${STORY_DIR}/pitch.md`. Si un fichier d'intention requis manque, **arrête-toi** et redirige vers la skill de cadrage appropriée (`/forge:feature-pitch`, `/forge:refactor-plan`, `/forge:tech-plan`).
...
Catégories (déjà documentées dans `forge:report` SKILL.md §Phase 3, qui reste la référence) :
...
> - `/forge:commit` pour committer la clôture documentaire
