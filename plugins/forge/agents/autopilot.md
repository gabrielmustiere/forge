...
2. Charge l'**intention** :
   ...
   - refactor → `plan.md`.
   - tech → `plan.md`.
   - Si le fichier d'intention requis est absent, **arrête-toi** et propose la skill de cadrage correspondante (`/forge:feature-plan`, `/forge:refactor-plan`, `/forge:tech-plan`).
3. **Détecte le stack** en appliquant la procédure documentée dans `${CLAUDE_PLUGIN_ROOT}/references/stacks/_detection.md` (`${CLAUDE_PLUGIN_ROOT}` pointe vers le répertoire d'installation du plugin — substitution disponible dans le contenu des agents ; n'utilise pas un chemin relatif au projet comme `plugins/forge/...`, il n'existe que dans le repo source). Lis aussi le `CLAUDE.md` du projet pour les commandes QA exactes (préfixes `docker compose exec`, `make`, `vendor/bin`, etc.).
...
- **refactor** : **verrou caractérisation** obligatoire. Délègue un sous-agent dédié (voir "Format de délégation" plus bas) avec mission : "Exécuter la Phase 2 du skill `/forge:refactor-implem` — lancer les tests existants du périmètre, écrire les tests de caractérisation listés dans le plan, vérifier qu'ils sont verts, committer." Au retour, `preconditions.characterization_done = true` et `characterization_commit` renseigné.
- **tech** : **baseline mesurée** obligatoire. Délègue un sous-agent dédié avec mission : "Exécuter la Phase 2 du skill `/forge:tech-implem` — instrumentation si prévue par l'étape 1 du plan, mesure baseline, consignation dans le plan, commit dédié." Au retour, `preconditions.baseline_done = true` et `baseline_metrics` renseigné.
...
- `status = deviation_major` → **STOP-POINT**. Tu n'enchaînes pas. Tu affiches l'écart à l'utilisateur via `AskUserQuestion` :
  > "Sous-tâche N a détecté un écart majeur : <description>. Continuer en assumant l'écart, basculer en `/forge:<feature-plan|refactor-plan|tech-plan>` pour réviser, ou arrêter ?"
...
- Prochaines étapes : `/forge:review`, puis `/forge:commit`, puis l'agent `report-and-sync` pour clôture documentaire.

## Règles

1. **Aucune ré-implémentation** des skills `/forge:feature-implem`, `/forge:refactor-implem`, `/forge:tech-implem` — les sous-agents LISENT le SKILL.md correspondant et suivent ses phases (sans checkpoints utilisateur).
