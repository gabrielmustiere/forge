...
- **Exigence réglementaire honorée** : sécurité des tokens d'accès (chiffrés au repos, jamais réaffichés, jamais loggés).
- **Impact North Star** : indirect mais bloquant — sans projets déclarés, aucun kanban ne peut exister.

## Utilisateurs concernés

- **Utilisateur local connecté** (l'unique utilisateur de l'app, mono-utilisateur — cf. anti-objectif vision « backend partagé / multi-utilisateur ») — c'est lui qui déclare, consulte, édite et retire ses projets. Toute la feature vit derrière le firewall de `login`.
...
- En tant qu'**utilisateur connecté**, je veux **NE JAMAIS revoir mon token en clair** après l'avoir saisi, afin que ce secret ne fuite pas à l'écran ou dans le HTML de la page.
...
2. Le **nom est déduit par parsing de l'URL** (`owner/repo`) et pré-rempli ; l'utilisateur peut le corriger avant d'enregistrer (fallback manuel pour les URL atypiques). Le nom n'est jamais lu dans le contenu du repo à ce stade.
3. L'**URL est normalisée** avant stockage et comparaison (formes https/ssh, avec ou sans suffixe `.git` → forme canonique unique).
4. **Unicité** : deux projets ne peuvent pas pointer vers la même URL normalisée. Une déclaration en doublon est refusée avec un message explicite (« ce repo est déjà suivi »).
5. Le **token est chiffré au repos**, n'est **jamais réaffiché en clair** après saisie, n'est **jamais renvoyé au navigateur** (HTML/JS) et **n'apparaît jamais dans les logs**.
...
8. La **suppression est définitive** et passe par une confirmation explicite. Rien de scanné n'étant persisté en D2, aucune donnée dérivée n'est à préserver.
...
- [ ] Déclarer une URL déjà suivie (même forme normalisée : https/ssh, avec/sans `.git`) est refusé avec un message « ce repo est déjà suivi ».
...
- [ ] Le token n'est jamais présent dans le HTML servi (vérifiable en inspectant la source de la page d'édition) ni dans les logs applicatifs.
...
- **Lecture / scan du repo distant (D3)** : aucun appel réseau vers GitHub/GitLab. La déclaration est une donnée purement locale à l'app.
...
- **Multi-thème** : non.
- **i18n / traduction** : libellés UI en français (formulaire de déclaration/édition, liste, messages d'erreur, confirmation de suppression) — pas de contenu multilingue.
- **API** : non (aucune ressource API exposée ; l'app reste server-rendered).
