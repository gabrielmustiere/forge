...
C'est **la première brique « productive » du pivot** : la story 008 a rapatrié le repo en local ; celle-ci s'en sert pour *agir*. Jusqu'ici, entrer dans le workflow forge — exprimer un besoin, le cadrer — imposait un terminal, un repo et une commande. Cette story ouvre une **porte d'entrée GUI** : le besoin s'exprime dans l'app, le skill de cadrage tourne côté serveur, et le résultat atterrit versionné sur le repo, en revue.

La faisabilité technique de l'exécution headless a été **prouvée par un POC** (interview 3-tours produisant un `brief.md` réel pour ~0,13 $) et la décision d'architecture gravée en **ADR-0002** (exécution via le CLI `claude -p` headless + clé API). Cette story cadre le **fonctionnel** de ce parcours ; elle ne rejoue pas ce choix technique.
...
- **Principes respectés** : #1 « l'app agit mais reste bornée » (cadrage only, brouillon jamais mergé) ; #2 « cadrage only — jamais de code » (le brief est 100 % fonctionnel, aucun `src/` touché) ; #3 « projection lecture-seule préservée » (branche dédiée, `main` et le kanban intacts) ; #4 « la convention forge est le contrat » (`brief.md`, `docs/story/NNN-f-<slug>/`) ; #5 « prouver avant d'ouvrir » (POC mono-utilisateur).
...
8. **Accès en écriture requis** : le dépôt réutilise le **token stocké du projet**, qui doit désormais disposer du **droit d'écriture**. Si le token n'a que la lecture (ou en cas d'échec réseau / conflit), le parcours affiche un **échec lisible** et le brief produit **n'est pas perdu** (il reste disponible en local pour re-tenter). Le token n'apparaît jamais en clair (logs, config git du clone).
9. **GitHub d'abord** : l'ouverture de la proposition en brouillon est livrée **pour GitHub** dans cette story. GitLab est un suivant (le clone 008 supporte déjà les deux, mais l'ouverture de MR/PR diffère par provider).
...
- **Migration de données** : **oui** — persister l'état du parcours (statut de l'interview, référence de session de dialogue, identité de la story produite, référence de la branche / proposition, raison d'échec éventuelle, horodatages). Nouvelle entité dédiée ou champs — tranché au plan.
- **i18n / traduction** : oui — libellés du bouton, des états et des messages du parcours.
- **Permissions** : inchangé au sens des rôles (tout vit derrière le firewall `login`), mais **nouvelle exigence de scope** sur le token du projet (écriture) — à surfacer côté déclaration/édition du projet.
- **Sécurité** : sensible — exécution d'un agent avec accès fichiers sur le serveur (à borner : outils restreits, pas de réseau hors fournisseur IA — cf. ADR-0002) **et** manipulation d'un token d'écriture (jamais en clair). Un secret d'app (clé du fournisseur IA) est requis côté serveur.
- **API** : **oui, sortante** — appel de l'API GitHub pour ouvrir la proposition en brouillon (aucun endpoint public exposé côté Board).
- **Multi-tenant / multi-thème** : non (mono-utilisateur, POC).
