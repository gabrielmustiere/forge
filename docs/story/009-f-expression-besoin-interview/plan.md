...
- **Live Component + `DefaultActionTrait`** : composant `ProjectInterview` avec `LiveAction` (`start`, `submit`, `validate`, `abandon`) et **poll** sur les états `isInFlight()` (le worker fait le travail hors requête ; réhydratation de l'entité `LiveProp` à chaque cycle → pattern `ProjectCloneStatus`).
...
| `src/Repository/InterviewRepository.php`                       | `findActiveForProject()` (statut non terminal) — requête cloisonnée au repository.     |
| `src/Manager/InterviewManager.php`                             | Orchestration : `start/submitMessage/submitBrief/abandon`, gardes + dispatch async.    |
...
| `src/Service/Interview/GitBriefPusher.php`                  | Copie de travail isolée + branche + commit + push via `GIT_ASKPASS`.                   |
...
3. [ ] `InterviewRepository::findActiveForProject()`.
...
- **Le brief est écrit dans le clone maintenu (008)** : le skill tourne avec `cwd` = clone → fichiers **non suivis** ajoutés dans `private/<owner>-<repo>/docs/story/…`. Sans effet sur le `git pull --ff-only` de la 008 (branche par défaut inchangée). Risque résiduel : si la PR est mergée plus tard, un pull pourrait entrer en conflit avec la copie locale non suivie → à surveiller (nettoyage post-submit candidat).
...
- **Reprise d'une interview interrompue** : le design la rend quasi gratuite (entité + session `claude` persistées → rouvrir la page réhydrate le fil et continue). Confirmer qu'on l'expose en V1 (le pitch la listait incertaine). → implem.
- **Nettoyage post-submit** du dossier de story non suivi dans le clone maintenu : le supprimer après PR ouverte, ou le garder pour re-tenter ? → implem.
- **Modèle par défaut** : Haiku (POC) vs Sonnet si la reconnaissance de code sur un vrai repo l'exige (hypothèse vision #2). Configurable via `CLAUDE_MODEL`. → à éprouver.
- **Copie isolée : mécanisme** : `git clone --local` du clone maintenu (rapide, partage d'objets) + recopie du dossier de story non suivi, vs worktree. → tranché à l'implem (principe : clone 008 intact).
- **Extraction testable de la construction de commande** `claude` : en faire une méthode pure testée unitairement, ou s'en remettre aux doubles ? → build.
- **Fake runner en env `dev`** (pour l'E2E CI, comme `DevFakeRepositoryReader`) : à câbler si l'E2E doit tourner sans `claude`. → implem.
