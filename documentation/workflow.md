...
| [`estimate`](../plugins/workflow/skills/estimate/SKILL.md) | **Transversal optionnel** — chiffre le temps **« tout compris »** d'une story (feature, refacto, tech) à facturer : cadrage, implem, tests, review, intégration, doc, release, coordination. Lit `brief.md`/`pitch.md`/`plan.md` selon ce qui existe, méthode réaliste + marge d'incertitude, **en heures**, avec **deux colonnes** (temps de référence sans IA / temps réel avec assistant IA) → `estimate.md`. Du temps, jamais de montant. |
